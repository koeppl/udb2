...
for ex in c++11/run-test separate_chaining/run-test-cht separate_chaining/run-test-compact separate_chaining/run-test-plain sparsehash/run-test tsl_sparse_map/run-test sparsepp/run-test rigtorp/run-test; do
	[[ -x $ex ]] || continue
	name=$(dirname "$ex")
